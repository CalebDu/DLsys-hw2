...
def one_hot(shape, i, device=None, dtype="float32", requires_grad=False):
    """ Generate one-hot encoding Tensor """
    device = ndl.cpu() if device is None else device
    return ndl.Tensor(device.one_hot(shape,i.numpy(), dtype=dtype), device=device, requires_grad=requires_grad)
